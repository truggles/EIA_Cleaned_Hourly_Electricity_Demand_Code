...
impute_file_path = paste0("data/csv_MASTER_",version,".csv")
...
csv_MASTER = csv_MASTER %>% dplyr::filter(as.Date(time) > "2015-07-01" &
                                          as.Date(time) < paste0(year_updated, "-07-02"))
...
data_wide_logged = data.frame(time = pre_imputed_data$time,
                              pre_imputed_data %>% dplyr::select(-time) %>% apply(MARGIN = 2, FUN = function(x) log(x)))
...
data_pre_impute0 =  data_wide_logged %>% data.table() %>%
                                         melt(, id.vars = c('time'), variable.name = "BA") %>% 
...
data_pre_impute_tack_front_end = data_pre_impute0 %>% dplyr::filter(doy <= (15-1)/2) %>%
                                                      dplyr::mutate(original = 0,
                                                                    doy = ifelse(lubridate::year(time) %in% c(leap_years),
                                                                                 doy + 366,
                                                                                 doy + 365))

data_pre_impute_tack_back_end = data_pre_impute0 %>% dplyr::filter(ifelse(lubridate::year(time) %in% c(leap_years), 
                                                                          doy > 366 - (15-1)/2,
                                                                          doy > 365 - (15-1)/2)) %>%
                                                     dplyr::mutate(original = 0,
                                                                   doy = ifelse(lubridate::year(time) %in% c(leap_years),
...
data_pre_impute = rbind(data_pre_impute_tack_front_end,
                  ...
                  dplyr::arrange(BA,time) %>%
                  dplyr::group_by(BA) %>%
                  dplyr::mutate(demand_lag1 = lag(value, na.rm = TRUE),
                                demand_lead1 = lead(value, na.rm = TRUE),
                                time = as.POSIXct(time, tryFormats = c("%Y-%m-%d %H:%M"), tz = 'UTC')) %>%
                  ungroup() %>%
  dplyr::select(-doy, -hour, -original)

# # save this for later use of the demand_clim_mean variable during the over imputation stage
# clim_var_to_save = data_pre_impute %>% dplyr::select(time, BA, demand_clim_mean)
# save(clim_var_to_save, 
#      file = 'Google Drive/MICE_Impute_EIA_Demand_Data/Clim_var/demand_clim_mean_Jan15.RData')
# rm(clim_var_to_save)
...
data_pre_impute_wide = merge(merge(merge(reshape2::dcast(data = data_pre_impute, time ~ BA_concurrent_label, value.var = 'value', fun = mean),
                                         reshape2::dcast(data = data_pre_impute, time ~ BA_clim_mean_label, value.var = 'demand_clim_mean', fun = mean),
                                         by = 'time'),
                                   reshape2::dcast(data = data_pre_impute, time ~ BA_lag1_label, value.var = 'demand_lag1', fun = mean),
                                   by = 'time'),
                             reshape2::dcast(data = data_pre_impute, time ~ BA_lead1_label, value.var = 'demand_lead1', fun = mean),
                             by = 'time')
...
pred_mat = make.predictorMatrix(data_pre_impute_wide %>% dplyr::select(-time))
...
time_now_check = proc.time()
# impute with mice
data_imp_mice_lag_embed_par = parlmice(data = data_pre_impute_wide %>% dplyr::select(-time),
                                       n.imp.core = 4,
                                       maxit = 10,
...
for(iimp in 1:16){
    mice_lag_embed_imputations_temp = data.frame(time = data_pre_impute_wide$time,
...
mice_lag_embed_imputations_summarized = mice_lag_embed_imputations %>% melt(, id.vars = c('time', 'imp_index'), variable.name = 'BA', value.name = 'demand') %>%
  as.data.table() %>%
  dplyr::group_by(time, BA) %>% 
  dplyr::summarise(demand = as.integer(round(mean(exp(demand), na.rm = TRUE),0))) %>%
  ungroup()

mice_lag_embed_imputations_summarized_wide = mice_lag_embed_imputations_summarized %>% reshape2::dcast(time ~ BA, value.var = 'demand', fun = mean)
...
mice_lag_embed_imputations_all = mice_lag_embed_imputations %>% melt(, id.vars = c('time', 'imp_index'), variable.name = 'BA', value.name = 'demand') %>%
  as.data.table() %>%
  dplyr::group_by(time, BA, imp_index) %>% 
  dplyr::mutate(demand = as.integer(round(exp(demand),0))) %>%
  ungroup()

mice_lag_embed_imputations_all_wide = mice_lag_embed_imputations_all %>% reshape2::dcast(time + imp_index ~ BA, value.var = 'demand', fun = mean)
