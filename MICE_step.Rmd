...
data_pre_impute0 =  data_wide_logged %>% data.table() %>%
                                         melt(, id.vars = c('date_time'), variable.name = "BA") %>% 
...
# clim_var_to_save = data_pre_impute %>% dplyr::select(date_time, BA, demand_clim_mean)
...
mice_lag_embed_imputations_summarized = mice_lag_embed_imputations %>% melt(, id.vars = c('date_time', 'imp_index'), variable.name = 'BA', value.name = 'demand') %>%
  as.data.table() %>%
...
mice_lag_embed_imputations_all = mice_lag_embed_imputations %>% melt(, id.vars = c('date_time', 'imp_index'), variable.name = 'BA', value.name = 'demand') %>%
  as.data.table() %>%
